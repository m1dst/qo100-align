...
const supportedLangs = new Set(Array.from(langSelectEl?.options || []).map((opt) => opt.value));
const i18nCache = new Map();
let languageApplySeq = 0;

async function loadLanguageData(lang) {
  if (!lang || !supportedLangs.has(lang)) return null;
  if (i18nCache.has(lang)) return i18nCache.get(lang);

  const response = await fetch(`./i18n/${lang}.json`, { cache: "no-store" });
  if (!response.ok) {
    throw new Error(`Failed to load language file for ${lang}: HTTP ${response.status}`);
  }
...
function t(key, vars = {}) {
  const enDict = i18nCache.get(DEFAULT_LANG) || {};
  const dict = i18nCache.get(currentLang) || enDict;
...
function formatNumber(value, options = {}) {
  ...
  const locale = supportedLangs.has(currentLang) ? currentLang : DEFAULT_LANG;
  const fmt = new Intl.NumberFormat(locale, options);
  return fmt.format(n);
}

async function applyLanguage(lang) {
  const desiredLang = supportedLangs.has(lang) ? lang : DEFAULT_LANG;
  const seq = ++languageApplySeq;

  try {
    await loadLanguageData(DEFAULT_LANG);
    if (desiredLang !== DEFAULT_LANG) {
      await loadLanguageData(desiredLang);
    }
  } catch (err) {
    console.error(err);
  }

  if (seq !== languageApplySeq) return;

  currentLang = i18nCache.has(desiredLang) ? desiredLang : DEFAULT_LANG;
...
const initialLang = supportedLangs.has(savedLang) ? savedLang : (supportedLangs.has(browserLang) ? browserLang : DEFAULT_LANG);
if (langSelectEl) {
  langSelectEl.value = initialLang;
  langSelectEl.addEventListener("change", () => {
    void applyLanguage(langSelectEl.value);
  });
}
void applyLanguage(initialLang);
